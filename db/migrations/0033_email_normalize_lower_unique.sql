...
-- invitations.email gilt seit 0021 als kanonisch (delete.ts/konto-sicherheit
-- vergleichen dagegen), aber Alt-Bestand aus Vor-0021-Zeiten könnte MixedCase/
-- Whitespace tragen. Hier mit derselben kanonischen Form nachziehen.
UPDATE "invitations" SET "email" = lower(btrim("email")) WHERE "email" <> lower(btrim("email"));--> statement-breakpoint
...
-- 4) Funktionaler UNIQUE-Index — DB-seitiges Netz für die Kanonizität. Der
--    Ausdruck ist bewusst lower(btrim(email)) — DECKUNGSGLEICH mit Wächter (1)
--    und Backfill (2): sonst entgingen Whitespace-Zwillinge dem DB-Netz. SQL ist
--    hier die Quelle der Wahrheit; schema.ts spiegelt denselben Ausdruck.
CREATE UNIQUE INDEX "users_tenant_email_lower_unique" ON "users" USING btree ("tenant_id",lower(btrim("email")));--> statement-breakpoint
