...
export default async function AdminDashboardPage({ params }: PageProps) {
  ...
  const digestEntwurfCount = digestEntwurfRows[0]?.count ?? 0;

  // PII-freie Civic-Kennzahlen fürs Dashboard (P2 §Empf. 5) — enthält auch die
  // offenen Anliegen (für die Karten-Badge wiederverwendet).
  const kennzahlen = await getAdminKennzahlen(db, tenant.id);
  const anliegenOffenCount = kennzahlen.offeneAnliegen;
  ...
  return (
    <main className="min-h-screen px-6 py-10 max-w-4xl mx-auto">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {/* Karte: Abstimmungen (M5 Composer + Lebenszyklus) */}
        <Link
        ...
        >
          ...
          <p className="text-sm text-zinc-500">
            Abstimmungen für Kommune oder Ortsteil erstellen, aktivieren und
            schließen. Verbindliche Abstimmungen nur für verifizierte Bürger:innen.
          </p>
          <p className="mt-3 text-sm font-medium text-[var(--tenant-primary,var(--pz-brand))] group-hover:underline">
